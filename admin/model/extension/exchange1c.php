...
class ModelExtensionExchange1c extends Model {
	...
	private function checkCML($xml) {
		...
		if ($xml['ВерсияСхемы']) {
			...
			$this->ERROR = "Файл не является стандартом Commerce ML. Не указана версия схемы. Допустимые версии от 2.03 до 2.10";
			$this->ERROR_NO = "0101";
			return false;
		}
		return true;
	...
	public function deleteLinkProduct($product_id) {
		...
		$this->query("DELETE FROM `" .  DB_PREFIX . "product_unit` WHERE `product_id` = " . (int)$product_id);
		$this->log("Удалены единицы измерения товара", 2);
	...
	public function checkDB() {
		...
		$tables_module = array("product_to_1c","product_quantity","product_price","product_unit","category_to_1c","warehouse","product_feature","product_feature_value","store_to_1c","attribute_to_1c","manufacturer_to_1c","attribute_value");
...
//					}
	...
	private function addProduct(&$data) {
		...
		$data['status'] = $this->config->get('exchange1c_product_new_status_disable') == 1 ? 0 : 1;
	...
	private function setProductUnit($product_id, $unit_data, $product_feature_id) {
		...
		if (!$unit_id) {
			...
			$query = $this->query("SELECT `product_unit_id` FROM `" . DB_PREFIX . "product_unit` WHERE `product_id` = " . (int)$product_id . " AND `product_feature_id` = " . (int)$product_feature_id . " AND `unit_id` = " . (int)$unit_id . " AND `ratio` = " . $unit_data['ratio']);
			if ($query->num_rows) {
		...
		if (!$product_unit_id) {
			...
			$this->query("INSERT INTO `" . DB_PREFIX . "product_unit` SET `product_id` = " . (int)$product_id . ", `product_feature_id` = " . (int)$product_feature_id . ", `unit_id` = " . (int)$unit_id . ", `ratio` = " . $unit_data['ratio']);
			$product_unit_id = $this->db->getLastId();
		}
		...
		if ($data_price['table_price'] == 'discount') {
			...
			if ($found_price) {
				...
				if ($update_fields) {
					...
					if (isset($update_fields['customer_group_id'])) $sql[] = "`customer_group_id` = " . $update_fields['customer_group_id'];
					$sql = implode(", ", $sql);
					$this->query("UPDATE `" . DB_PREFIX . "special` SET " . $sql . " WHERE `product_special_id` = " . (int)$product_price_id);
					// Обновим цену
				}
	...
	private function getProductUnits($product_id) {
		...
		$query = $this->query("SELECT * FROM `" .  DB_PREFIX . "product_unit` WHERE `product_id` = " . (int)$product_id);

		if ($query->num_rows) {

			foreach ($query->rows as $row) {

				$result[$row['product_unit_id']] = array(
	...
	private function updateOffers($product_id, $data, $old_data) {
		...
		if (isset($data['product_feature_id'])) {
			...
			if (isset($data['options'])) {
				...
				foreach ($data['options'] as $option) {
					...
					$this->log($product_option_value_id, 2);
		...
		if (isset($data['units'])) {
			...
			$old_units = $this->getProductUnits($product_id);

			foreach ($data['units'] as $unit) {

				$update_fields = $this->compareArraysData($data['unit'], $old_units);
	 ...
	 */
 	private function setProduct(&$data) {
		...
		// Проверка на ошибки
		if (empty($data)) {
			$this->ERROR = "Нет данных о товаре";
			return false;
		}
			...
			switch ($name){
				...
				case 'Полное наименование':
					if ($value && $this->config->get('exchange1c_product_name') == 'fullname') {
						...
						$this->log("Наименование товара изменено из реквизита: " . $name . " = " . $value, 2);
					}
				break;
				case 'Производитель':
		...
		foreach ($xml as $image) {
			...
			if (!file_exists(DIR_IMAGE . $image)) {
				...
				if ($this->config->get('exchange1c_product_images_check') == 1) {
					$this->log("parseImages() - файл не существуети не будет обработан далее: " . $image);
					continue;
				}
				...
				if ($image_info == NULL) {
					$this->log("ВНИМАНИЕ! файл не является картинкой: " . DIR_IMAGE . $image);
				};
			}
...
//		$this->log($attribute_id, 2);
	...
	private function parseClassifierAttributes($xml) {
		...
		// Не выбран ни один тип
		$this->log(count($types), 2);
		if (count($types) == 3) {
			$this->log("ВНИМАНИЕ! Не выбран ни один тип свойства, загрузка прервана");
			return 0;
		}
		...
		foreach ($properties as $property) {
			...
			if ($property->ВариантыЗначений->Справочник) {
				foreach ($property->ВариантыЗначений->Справочник as $value) {
	 				$values[(string)$value->ИдЗначения] = trim(htmlspecialchars((string)$value->Значение));
				}
			}
			$this->log($values, 2);
		...
		$this->log("********************************************");
		$this->log("Атрибутов прочитано из файла: " . count($data), 2);
	...
	private function parseProductAttributes($xml, &$data) {
		...
		$manufacturer_tag = $this->config->get('exchange1c_product_manufacturer_tag');
		$this->log("Загрузка производителя из элемента в XML: '" . $manufacturer_tag . "'", 2);
	...
	private function addManufacturer($data) {
		...
		$this->query("INSERT INTO `" . DB_PREFIX . "manufacturer_to_store` SET `manufacturer_id` = " . (int)$manufacturer_id . ", `store_id` = " . $this->STORE_ID);
 		...
 		$this->log("Производитель добавлен: '" . $data['name'] . "', manufacturer_id: " . $manufacturer_id, 2);
	...
	private function setManufacturer($name, $manufacturer_guid = '') {
		...
		if ($manufacturer_guid) {
			if (isset($this->MANUFACTURERS[$manufacturer_guid])) {
				$manufacturer_id = $this->MANUFACTURERS[$manufacturer_guid];
			}
		}

		$sql_select = "SELECT `m1c`.`manufacturer_id`, `m1c`.`guid`, `m`.`image`, `m`.`sort_order`";
		$sql_from = " FROM `" . DB_PREFIX . "manufacturer_to_1c` `m1c` LEFT JOIN `" . DB_PREFIX . "manufacturer` `m` ON(`m1c`.`manufacturer_id` = `m`.`manufacturer_id`) LEFT JOIN `" . DB_PREFIX . "manufacturer_to_store` `ms` ON(`m1c`.`manufacturer_id` = `ms`.`manufacturer_id`)";
		$sq_where = " WHERE `ms`.`store_id` = " . $this->STORE_ID;

		// Поищем производителя
		if (isset($this->TAB_FIELDS['manufacturer_description']['name'])) {
			$sql_select .= ", `md`.`name`";
			$sql_from .= " LEFT JOIN `" . DB_PREFIX . "manufacturer_description` `md` ON(`m1c`.`manufacturer_id` = `md`.`manufacturer_id`)";
			if ($manufacturer_id) {
				$sq_where .= " AND `m`.`manufacturer_id` = " . $manufacturer_id;
			} elseif ($manufacturer_guid) {
				$sq_where .= " AND `m1c`.`guid` = '" . $this->db->escape($manufacturer_guid) . "'";
			} else {
				$sq_where .= " AND `md`.`name` = '" . $this->db->escape($name) . "'";
			}
		} else {
			$sql_select .= ", `m`.`name`";
			if ($manufacturer_id) {
				...
				$sq_where .= " AND `m`.`name` = '" . $this->db->escape($name) . "'";
			}
		}

		$query = $this->query($sql_select . $sql_from . $sq_where);
		...
		if ($query->num_rows) {
			...
			$this->updateManufacturer($manufacturer_id, $data, $name);

		} else {

			$data = array(
				'name'			=> trim(htmlspecialchars((string)$name)),
				'image'			=> "",
				'sort_order'	=> 0,
				'guid'			=> $manufacturer_guid
			);
			$manufacturer_id = $this->addManufacturer($data);
			$this->MANUFACTURERS[$manufacturer_guid] = $manufacturer_id;
		}
	...
	private function parseProductUnit($xml = null) {
		...
		if ($xml['Код']) {
			$data['code']		= trim((int)$xml['Код']);
			$data['name']		= (string)$xml;
			$data['full_name']	= (string)$xml['НаименованиеПолное'];
		} else {
			$data['code'] 		= trim((int)$xml);
			$data['name'] 		= "шт";
		...
		$this->log("Единица измерения, код: '" . $data['code'] . "', коэффициент: " . $data['ratio']);
	...
	private function parseClassifierUnits($xml) {
		...
		foreach ($xml->ЕдиницаИзмерения as $unit) {
			...
			if (isset($this->UNITS[$guid])) {

				$unit_id = $this->UNITS[$guid]['unit_id'];
				$this->log($this->UNITS[$guid], 2);

				$update_fields = $this->compareArraysData($data, $this->UNITS[$guid]);
	...
	private function getManufacturers() {
		...
		$query = $this->query("SELECT `manufacturer_id`, `guid` FROM `" . DB_PREFIX . "manufacturer_to_1c`");

    	if ($query->num_rows) {

	    	foreach ($query->rows as $row) {

	    		$this->MANUFACTURERS[$row['guid']] = $row['manufacturer_id'];
	...
	private function parseProductManufacturer($product, &$data) {
		...
		if ($product->$manufacturer_tag) {
			...
			$data['manufacturer_guid'] = (string)$product->$manufacturer_tag->Ид;
        	$data['manufacturer_name'] = htmlspecialchars(trim((string)$product->Изготовитель->Наименование));
	...
	private function parseProductTaxes($product) {
		...
		foreach ($product->СтавкаНалога as $product_tax) {
			...
			if ($query->num_rows) {
				...
				$this->ERROR = "Не найден налог с наименованием '" . $name . "'";
				return false;
			}

			if (!$rate) {
				$this->ERROR = "Не найдена ставка '" . $new_rate . "'% у налога '" . $name . "'";
				return false;
			}

			if (!$tax_class_id) {
				$this->ERROR = "Для налога '" . $name . "' не установлены правила";
				return false;
			}
	...
	private function parseProducts($xml) {
		...
		$this->STAT['product_parse'] = microtime(true);
	...
	private function getCurrencyId($code) {
		...
		$this->ERROR = "Невозможно определить валюту по коду: " . $code;
		return 0;
	...
	private function parseClassifierPriceType($xml) {
		...
		if (empty($config_price_type)) {
			$this->log("В настройках модуля не указано ни одной цены. Добавьте цены вручную или включите автозагрузку цен.");
			return false;
		}

		if (empty($config_currency)) {
			$this->log("В настройках модуля не сопоставлена валюта. Цены не будут загружены и заказы не будут выгружаться!");
			return false;
		}
	...
	private function getProductFeature($product_feature_guid) {
		...
		foreach ($query_feature->rows as $row_feature) {
			...
			$data[$row_feature['product_feature_id']] = array();
			$product_feature = &$data[$row_feature['product_feature_id']];

			$query_feature_value = $this->query("SELECT `product_option_id`, `product_option_value_id` FROM `" . DB_PREFIX . "product_feature_value` WHERE `product_feature_id` = " . (int)$row_feature['product_feature_id']);

			foreach ($query_feature_value->rows as $row_value) {

				if (!isset($product_feature[$row_value['product_option_id']])) {
					$product_feature[$row_value['product_option_id']] = array($row_value['product_option_value_id']);
				} else {
					array_push($product_feature[$row_value['product_option_id']], $row_value['product_option_value_id']);
				}
	 ...
	 * update 2017-09-24
	 */
	private function addFeature($product_id, $data) {

		$sql = "";
		if (isset($data['ean'])) $sql .= ", `ean` = '" . $data['ean'] . "'";
		if (isset($data['sku'])) $sql .= ", `sku` = '" . $this->db->escape($data['sku']) . "'";
		$this->query("INSERT INTO `" . DB_PREFIX . "product_feature` SET `product_id` = " . $product_id . ", `guid` = '" . $this->db->escape($data['feature_guid']) . "'" . $sql);
	 ...
	 * ver 1
	 * update 2017-12-03
	 * Удаляет старые опции, которых нет в файле
	 */
	private function deleteProductOptions($options, $product_options) {

		foreach ($product_options as $product_option_id => $product_option) {
			if (isset($options[$product_option_id])) {
				// Проверим значения у этой опции
				if (!empty($product_option['values'])) {
					foreach ($product_option['values'] as $product_option_value_id => $value) {
						if (array_search($product_option_value_id, $options[$product_option_id]) === false) {
							$this->log("Удалим старое значение опции product_option_value_id = " . $product_option_value_id, 2);
				...
				// Нету такой опции у товара, удаляем вместе со значениями
				$this->log("Удалим у товара старую опцию product_option_id = " . $product_option_id, 2);
				$this->query("DELETE FROM `" . DB_PREFIX . "product_option` WHERE `product_option_id` = " . $product_option_id);
				// Удалим значения
				if (!empty($product_option['values'])) {
					foreach ($product_option['values'] as $product_option_value_id => $value) {
						$this->query("DELETE FROM `" . DB_PREFIX . "product_option_value` WHERE `product_option_value_id` = " . $product_option_value_id);
						$this->log("Удалим у опции старое значение product_option_value_id = " . $product_option_value_id, 2);
						// В характеристике
						$this->query("DELETE FROM `" . DB_PREFIX . "product_feature_value` WHERE `product_option_value_id` = " . $product_option_value_id);
					}
				}
	 ...
	 * ver 12
	 * update 2017-10-01
	 * Разбор характеристики из файла offers.xml
	 */
	private function parseOptions($xml, $product_id, $images) {

		$this->log("Чтение характеристик (options)", 2);

		if (empty($xml->ХарактеристикаТовара)) {
			$this->log("Нет характеристик");
			return false;
		}

		$options = array();

		if ($this->config->get('exchange1c_product_option_name')) {
			$ar_name = array($this->config->get('exchange1c_product_option_name'));
		} else {
			$ar_name = array();
		}
		$ar_value = array();
		...
		foreach ($xml->ХарактеристикаТовара as $product_option) {
			...
			// Ид может быть не всегда
			$guid = (string)$product_option->Ид;
			$option_name = trim(htmlspecialchars((string)$product_option->Наименование));
			$value_name = trim(htmlspecialchars((string)$product_option->Значение));
			$index = 0;

			// Если есть в конце скобочки, значит там указана к какой категории относится опция
			$split = $this->splitNameStr($option_name);
			if ($split['option']) {
				$option_name		= $split['name'];
				$product_category	= $split['option'];
				$this->log("Опция для категории товара: " . $product_category, 2);
				//$sort_order			= $split['order'];
			}

			//*** РЕЖИМЫ ЗАГРУЗКИ ХАРАКТЕРИСТИК ***
			// Стандартный режим. Как в УС характеристика и несколько значений
			if ($this->config->get('exchange1c_product_options_mode') == 'feature') {
				...
				$options[] = array(
					'option_id'		=> $option_id,
					'name'			=> $option_name,
					'value_id'		=> $this->setOptionValue($value_name, $option_id, $image),
					'value'			=> $value_name,
					'subtract'		=> $this->config->get('exchange1c_product_options_subtract') == 1 ? 1 : 0
				);
		...
		// В режиме характеристик, будет только один вариант, т.е. одна опция
		if ($this->config->get('exchange1c_product_options_mode') == 'feature') {

			$option_name = implode(",",$ar_name);
			$value_name = implode(",",$ar_value);
	...
	private function parseOffers($xml) {
		...
		$this->STAT['offers'] = microtime(true);
		...
		foreach ($xml->Предложение as $offer) {
			...
			if (!$product_id) {
				$this->log("Товар не найден по Ид '" . $data['product_guid'] . "', предложение игнорируется!", 2);
				continue;
			}

			$old_product = $this->getProduct($product_id);

			//$data['name'] = $offer->Наименование ? htmlspecialchars(trim((string)$offer->Наименование)) : "";

			$this->log("ПРЕДЛОЖЕНИЕ ТОВАРА Ид: " . $data['product_guid'] . ", product_id = " . $product_id, 2);
			...
			if ($data['feature_guid']) {
				...
				if ($old_feature) {
					...
					$data['product_feature_id'] = $this->addFeature($product_id, $data);
				}
			} else {
			...
			// Обновляем товар
			$this->STAT['update_offers'] = microtime(true);
			$update = $this->updateOffers($product_id, $data, $old_product);
			$this->logStat('update_offers');
			if ($this->ERROR) return false;
...
//		$requisites['Склад']						= 'Основной склад'
	...
	private function addOrderProduct($order_id, $product_id, $price, $quantity, $total, $tax = 0, $reward = 0) {
		...
		$query = $this->query("SELECT `pd`.`name`,`p`.`model` FROM `" . DB_PREFIX . "product` `p` LEFT JOIN `" . DB_PREFIX . "product_description` `pd` ON (`p`.`product_id` = `pd`.`product_id`) WHERE `p`.`product_id` = " . (int)$product_id);
		if ($query->num_rows) {
			$name = $query->row['name'];
			...
			$this->query("INSERT INTO `" . DB_PREFIX . "order_product` SET `product_id` = " . (int)$product_id . ",
				`order_id` = " . (int)$order_id . ",
				`name` = '" . $this->db->escape($name) . "',
				`model` = '" . $this->db->escape($model) . "',
				`price` = " . (float)$price . ",
				`quantity` = " . (float)$quantity . ",
				`total` = " . (float)$total . $sql);
			return $this->db->getLastId();
		}
		return 0;
	...
	private function changeOrderStatus($order_id, $status_name) {
		...
		$this->query("INSERT INTO `" . DB_PREFIX . "order_history` SET `order_id` = " . (int)$order_id . ", `order_status_id` = " . (int)$new_order_status_id . ", `date_added` = '" . $this->NOW . "'");
		...
		if (!empty($doc['products'])) {
			...
			foreach ($doc['products'] as $key => $doc_product) {
				...
				if ($order_product_fields) {
					...
					if ($update) {
						$this->query("UPDATE `" . DB_PREFIX . "order_product` SET " . $fields . " WHERE `order_product_id` = " . (int)$products[$key]['order_product_id']);
	 ...
	 */
	private function parseClassifierCategories($xml, $parent_id = 0) {

		if (empty($xml)) {
			$this->ERROR = "parseClassifierCategories() - нет данных в XML";
			return false;
		}

		$this->log("Категорий в файле: " . count($xml->Группа));
		$this->STAT['category_num'] = count($xml->Группа);
		...
		foreach ($xml->Группа as $xml_category) {
			...
			if ($xml_category->Группы) {
				$this->parseClassifierCategories($xml_category->Группы, $category_id);
				if ($this->ERROR) return false;
			}

			$this->log("Обработана категория: '" . $data['name'] . "'");
	...
	private function parseClassifier($xml) {
		...
		if ($xml->Группы && $this->config->get('exchange1c_categories_no_import') != 1) {
			...
			$this->STAT['categories_parse'] = microtime(true);
			$num = $this->parseClassifierCategories($xml->Группы, 0);
			$this->logStat('categories_parse');

			if ($this->ERROR) return false;

			unset($xml->Группы);

			$this->log("Категорий загружено: " . count($num), 2);
			$this->log("*****");
		...
		if ($xml->Свойства) {
			...
			$this->STAT['attributes_parse'] = microtime(true);
			$num = $this->parseClassifierAttributes($xml->Свойства);
			$this->log($this->ATTRIBUTES, 2);
			$this->logStat('attributes_parse');
	...
	public function importFile($importFile, $type) {
		...
		if (is_file($importFile)) {
			...
			if (!$xml) {
				$this->ERROR = "ОШИБКА! Файл не является стандартом XML или обрезан\n";
				$this->ERROR .= implode("\n", libxml_get_errors());
				return $this->error();
			}

		} else {
			$this->ERROR = "ОШИБКА! Файл не был загружен на сайт, проверьте права на запись в папке /system/storage/cache/exchange1c.";
			return $this->error();
		}
	...
	public function defineTableFields() {
		...
		$tables = array(
			...
			'order_product'				=> array('product_feature_id'=>0,'product_unit_id'=>0),
			'customer'					=> array('patronymic'=>''),
			'cart'						=> array('product_feature_id'=>0,'product_unit_id'=>0),
