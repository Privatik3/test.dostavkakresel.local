<?php class ModelExtensionModuleUniGallery extends Model {
	...
	public function install() {
		$table = $this->db->query("show tables FROM `".DB_DATABASE."` LIKE '".DB_PREFIX."gallery'");
		$table1 = $this->db->query("show tables FROM `".DB_DATABASE."` LIKE '".DB_PREFIX."uni_gallery'");
		
		if($table->num_rows && !$table1->num_rows) {
			$this->db->query("RENAME TABLE `".DB_PREFIX."gallery` TO ".DB_PREFIX."uni_gallery");
			$this->db->query("RENAME TABLE `".DB_PREFIX."gallery_image` TO ".DB_PREFIX."uni_gallery_image");
			$this->db->query("RENAME TABLE `".DB_PREFIX."gallery_image_description` TO ".DB_PREFIX."uni_gallery_image_description");
		}
		...
		$query_name = $this->db->query("SELECT query FROM `".DB_PREFIX."url_alias` WHERE `keyword` LIKE 'gallery' LIMIT 1");
		if ($query_name->num_rows == 0) {
			$this->db->query("INSERT INTO ".DB_PREFIX."url_alias SET query = 'extension/module/uni_gallery', keyword = 'gallery'");
		} else {
			$this->db->query("DELETE FROM ".DB_PREFIX."url_alias WHERE keyword = 'gallery'");
			$this->db->query("INSERT INTO ".DB_PREFIX."url_alias SET query = 'extension/module/uni_gallery', keyword = 'gallery'");
		}
	}
}
