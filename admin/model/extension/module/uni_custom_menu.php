...
class ModelExtensionModuleUniCustomMenu extends Model {
	public function addCustomMenu($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "custom_menu SET link = '".$this->db->escape($data['link'])."', image = '".$this->db->escape($data['image'])."', parent_id = '".(int)$data['parent_id']."', parent_parent_id = '".(int)$data['parent_parent_id']."', `column` = '".(int)$data['column']."',status = '".(int)$data['status']."', sort_order = '".(int)$data['sort_order']."'");
	...
	public function editCustomMenu($menu_id, $data) {	
		$this->db->query("UPDATE " . DB_PREFIX . "custom_menu SET link = '".$this->db->escape($data['link'])."', image = '".$this->db->escape($data['image'])."', parent_id = '".(int)$data['parent_id']."', parent_parent_id = '".(int)$data['parent_parent_id']."' , `column` = '".(int)$data['column']."',status = '".(int)$data['status']."', sort_order = '".(int)$data['sort_order'] . "' WHERE menu_id = '".(int)$menu_id."'");
	...
	public function getCustomMenuForm($menu_id) {
		$sql = "SELECT * FROM " . DB_PREFIX . "custom_menu i LEFT JOIN " . DB_PREFIX . "custom_menu_description id ON (i.menu_id = id.menu_id) LEFT JOIN " . DB_PREFIX . "custom_menu_description id2 ON (parent_id = id2.menu_id) where i.menu_id='".(int)$menu_id."'";
