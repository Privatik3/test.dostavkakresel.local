      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          <ul class="nav nav-tabs">
            ...
            <li><a href="#tab-related-group" data-toggle="tab"><?php echo $tab_related_group; ?></a></li>
          </ul>
...
<div class="form-group hide">
            ...
            <div class="tab-pane" id="tab-option">
              <?php echo $fabrickchoice_tab;?></div>
              <?php if (false) { ?>
              <div class="row">
                    ...
                    <div class="tab-pane" id="tab-option<?php echo $option_row; ?>">
                      ...
                      <div class="table-responsive">
                        <table id="option-value<?php echo $option_row; ?>" class="table table-striped table-bordered table-hover">
                          <thead>
                            <tr>
                              <td style="width:200px;" class="text-left"><?php echo $entry_option_value; ?></td>
                              <td style="width:60px;" class="text-right">Кол-во</td>
                              <td style="width:60px;" class="text-left"><?php echo $entry_subtract; ?></td>
	        ...
	        <div class="tab-pane" id="tab-related-group">
		        <div class="form-group">
			        <label class="col-sm-2 control-label" for="input-group-mode"><?php echo $entry_group_mode; ?></label>
			        <div class="col-sm-10">
				        <select name="related_mode" id="input-group-mode" class="form-control">
					        <?php foreach ($related_modes as $key => $mode) { ?>
						        <option value="<?php echo $key;?>"<?php if ($key == $related_mode) { ?> selected="selected"<?php } ?>><?php echo $mode;?></option>
						    <?php } ?>
				        </select>
			        </div>
		        </div>
		        <div class="form-group">
			        <label class="col-sm-2 control-label" for="input-related-group"><span data-toggle="tooltip" title="<?php echo $help_related; ?>"><?php echo $entry_related; ?></span></label>
			        <div class="col-sm-10">
				        <input type="text" name="related-group" value="" placeholder="<?php echo $entry_related; ?>" id="input-related-group" class="form-control" />
				        <div id="product-all-product" class="well well-sm" style="height: 150px; overflow: auto;">
					        <?php foreach ($all_products as $product) { ?>
						        <div><i class="fa fa-plus-circle"></i> <?php echo $product['name']; ?>
							        <input type="hidden" value="<?php echo $product['product_id']; ?>" />
						        </div>
					        <?php } ?>
				        </div>
				        <div id="product-related-group" class="well well-sm" style="height: 150px; overflow: auto;">
					        <?php foreach ($product_relateds_group as $product_related) { ?>
						        <div id="product-related-group<?php echo $product_related['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product_related['name']; ?>
							        <input type="hidden" name="product_related_group[]" value="<?php echo $product_related['product_id']; ?>" />
						        </div>
					        <?php } ?>
				        </div>
			        </div>
...
$('#product-related').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
	   ...
	   // Related Group
	   $('input[name=\'related-group\']').on('keyup', function () {
		   var string = $(this).val();
		   if (!string) {
			   $('#product-all-product div').show();
		   } else {
			   var reg = new RegExp(string, 'i');
			   $('#product-all-product div').each(function () {
	   ...
	   $('#product-all-product').delegate('.fa-plus-circle', 'click', function() {

		   var item = {
			   'value' : $(this).parent().find('input').val(),
			   'label' : $(this).parent().text()
		   };
		   $('#product-related-group').append('<div id="product-related-group' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="product_related_group[]" value="' + item['value'] + '" /></div>');
		   $(this).parent().remove();

	   });

	   $('#product-related-group').delegate('.fa-minus-circle', 'click', function() {

		   var item = {
			   'value' : $(this).parent().find('input').val(),
			   'label' : $(this).parent().text()
		   };
		   $('#product-all-product').append('<div><i class="fa fa-plus-circle"></i> ' + item['label'] + '<input type="hidden" value="' + item['value'] + '" /></div>');
