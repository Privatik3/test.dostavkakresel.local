...
class ControllerCatalogProduct extends Controller {
    ...
    if ($this->config->get('config_editor_default')) {
		...
		$data['entry_group_mode'] = $this->language->get('entry_group_mode');
		...
		$data['tab_related_group'] = $this->language->get('tab_related_group');
    ...
    $data['manufacturers'] = $this->model_catalog_manufacturer->getManufacturers();
		...
		if (isset($this->request->post['related_mode'])) {
			$data['related_mode'] = $this->request->post['related_mode'];
		} elseif (!empty($product_info)) {
			$data['related_mode'] = $this->model_catalog_product->getProductRelatedMode($this->request->get['product_id']);
		} else {
			$data['related_mode'] = 0;
		}

		$data['related_modes'] = $this->language->get('related_modes');

		if (isset($this->request->post['product_related_group'])) {
			$products_related_group = $this->request->post['product_related_group'];
		} elseif (isset($this->request->get['product_id'])) {
			$products_related_group = $this->model_catalog_product->getProductRelatedGroup($this->request->get['product_id']);
		} else {
			$products_related_group = array();
		}

		$data['product_relateds_group'] = array();

		foreach ($products_related_group as $product_id) {
			$related_info = $this->model_catalog_product->getProduct($product_id);
		...
		foreach ($products as $product) {
			if (in_array($product['product_id'], $products_related_group)) {continue;}
