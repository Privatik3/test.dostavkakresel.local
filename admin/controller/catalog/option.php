	...
	protected function getForm() {
		...
		foreach ($option_values as $option_value) {
			...
			$data['option_values'][] = array(
				...
				'category_id'              => (isset($option_value['category_id'])) ? $option_value['category_id'] : ''
			);
		}
...
//		madmaxx
		...
		if ($this->config->get('fabrickchoice_status')) {
			$this->prepareCatgory($data);
		}
//      //
	...
	public function prepareCatgory(&$data) {
		...
		if (!isset($this->request->post['option_value'])) {
			...
			if ($values) {
				$categories = $this->model_extension_module_fabrickchoice->getCategoriesByValTable($values);
			}

			if ($categories) {
				foreach ($data['option_values'] as &$value) {

					if (array_key_exists($value['option_value_id'], $categories)) {
						$value['category_id'] = $categories[$value['option_value_id']];
					}
				}
			}
		}
	...
	protected function build_tree_category($cats, $parent_id = 0, $class = ''){

//		$this->load->model('module/fabrickchoice');
		$tree = array();
		if ($parent_id == 0) {
		...
		if(is_array($cats) and isset($cats[$parent_id])){
			...
			foreach($cats[$parent_id] as $cat){
				...
				$tree[$cat['category_id']] =  [
					...
					'child' => $this->build_tree_category($cats, $cat['category_id'])
				];
...
//				}
	...
	protected function renderButton($tree, $level = 0, $parent_name = '') {
//		echo '<pre>';
//		print_r($tree);
//		echo '<pre>';
		$html = '';
		...
		if ($level == 0) {
			$html .= '<button data-category="all" data-parent="0" class="btn btn-primary btn-danger">' . $this->language->get('text_showall') . '</button>';
			$html .= '<button data-category="no" data-parent="0" class="btn btn-primary">' . $this->language->get('text_nocat') . '</button>';
		}

//		if ($level) {
			$html .= '<div class="child_'. $level. ' cat_'.$level.' child">';
		if ($parent_name) {
			$html .= '<button class="btn lb" disabled>'. $parent_name .'</button>';
		}
//		}
...
//			$style = 'style="display: none;"';
		...
		$has_child = false;
		foreach ($tree as $cat) {
			$button .= '<button data-category="'.$cat['category_id'].'" data-parent="' . $level . '" class="btn btn-primary"' . $style . '>'. $cat['name'] . '</button>';
			if (isset($cat['child'])) {
				$has_child = true;
				$html_child .= $this->renderButton($cat['child'], $cat['category_id'], $cat['name']);
			}
		}

		if ($has_child) {
			$html .= '<span class="clps open"></span>' . $button;
		} else {
			$html .= $button;
		}

//		if ($level == 0) {
//			$html .= '<div class="hidden grid' . $level . ' child">';
//		} else {
			$html .= '<div class="hidden grid' . $level . ' child">';
//		}
		$html .= $html_child;
