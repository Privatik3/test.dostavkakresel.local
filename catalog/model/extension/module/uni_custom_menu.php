...
class ModelExtensionModuleUniCustomMenu extends Model {
	public function getCustomMenu(){
		...
		foreach($query->rows as $category){
			$query1 = $this->db->query("SELECT * FROM " . DB_PREFIX . "custom_menu h LEFT JOIN " . DB_PREFIX . "custom_menu_description hd ON (h.menu_id = hd.menu_id) where hd.language_id = '" . (int)$this->config->get('config_language_id') . "' and h.parent_id='".(int)$category['menu_id']."' and  h.status=1 order by h.sort_order");
			...
			foreach($query1->rows as $category1){
				...
				$query2 = $this->db->query("SELECT * FROM " . DB_PREFIX . "custom_menu h LEFT JOIN " . DB_PREFIX . "custom_menu_description hd ON (h.menu_id = hd.menu_id) where hd.language_id = '".(int)$this->config->get('config_language_id')."' and h.status=1 and h.parent_id='".(int)$category1['menu_id']."' order by h.sort_order");
