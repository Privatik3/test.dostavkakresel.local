	...
	public function index() {
		...
		if ($product_info) {
			...
			foreach ($results as $result) {
				...
				if ($mode_group == 1) {
					if (!$main_category = $this->model_catalog_product->getMainCategory($result['product_id'])) {
						continue;
					};

					$group_categories[] = $main_category['category_id'];
				} else {
							...
							foreach ($option['product_option_value'] as $option_value) {
								...
								if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
									...
									$product_option_value_data[] = array(
										...
										'default_select' => isset($option_value['default_select']) ? $option_value['default_select'] : '',
				...
				$data['products_group'][$main_category['category_id']][] = array(
			...
			$group_categories = array_unique($group_categories);
