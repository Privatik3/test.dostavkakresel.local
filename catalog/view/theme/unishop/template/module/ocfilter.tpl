				...
				<?php foreach ($selecteds as $option) { ?>
					<div class="ocfilter-option">
						...
						<?php foreach ($option['values'] as $value) { ?>
							<a onclick="location='<?php echo $value['href']; ?>';"><i class="fa fa-times"></i><?php echo $value['name']; ?></a>
						<?php } ?>
					</div>
				<?php } ?>
				<?php $count = count($selecteds); $selected = $selecteds; $first = array_shift($selected); ?>
				<?php if ($count > 1 || count($first['values']) > 1) { ?>
					<a onclick="location='<?php echo $link; ?>';" class="reset"><i class="fa fa-times-circle"></i><?php echo $text_cancel_all; ?></a>
				<?php } ?>
			</div>
		<?php } ?>
		<?php if ($show_price) { ?>
			...
			<div class="list-group-item ocfilter-option" id="option-<?php echo $option['option_id']; ?>">
				...
				<div class="option-values">
					...
					<?php } elseif ($option['type'] == 'slide' || $option['type'] == 'slide_dual') { # Slide type start ?>
						<div id="scale-<?php echo $option['option_id']; ?>" class="scale ocf-target" data-option-id="<?php echo $option['option_id']; ?>" data-start-min="<?php echo $option['slide_value_min_get']; ?>" data-start-max="<?php echo $option['slide_value_max_get']; ?>" data-range-min="<?php echo $option['slide_value_min']; ?>" data-range-max="<?php echo $option['slide_value_max']; ?>" data-element-min="#left-value-<?php echo $option['option_id']; ?>" data-element-max="#right-value-<?php echo $option['option_id']; ?>"></div>
					<?php } elseif ($option['type'] == 'radio' || $option['type'] == 'checkbox') { # Radio and Checkbox types start ?>
						<?php foreach ($option['values'] as $key => $value) { ?>
							...
							<?php } ?>
							<?php if ($option['color']) { ?><div class="color" style="background-color: #<?php echo $value['color']; ?>;"></div><?php } ?>
							<?php if ($option['image']) { ?><div class="image" style="background-image: url(<?php echo $value['image']; ?>);"></div><?php } ?>
							<?php if ($value['selected']) { ?>
								<label id="v-<?php echo $value['id']; ?>" class="input selected">
									...
									<?php if ($show_counter) { ?><small class="badge"></small><?php } ?>
								</label>
							<?php } elseif ($value['count']) { ?>
								<label id="v-<?php echo $value['id']; ?>" class="input">
									...
									<span></span>
									<a href="<?php echo $value['href']; ?>"><?php echo $value['name']; ?></a>
									<?php if ($show_counter) { ?><small class="badge"><?php echo $value['count']; ?></small><?php } ?>
								</label>
							<?php } else { ?>
								<label id="v-<?php echo $value['id']; ?>" class="input disabled">
									<input type="<?php echo $option['type']; ?>" name="ocfilter_filter[<?php echo $option['option_id']; ?>]" value="" disabled="disabled" class="ocf-target" />
									<span></span>
									<?php echo $value['name']; ?>
									<?php if ($show_counter) { ?><small class="badge">0</small><?php } ?>
								</label>
							<?php } ?>
							<?php if ($show_values_limit && $show_values_limit < count($option['values']) && $key + 1 == count($option['values'])) { ?>
								</div>
								<p><a href="#ocfilter-hidden-values-<?php echo $option['option_id']; ?>" data-toggle="collapse" aria-expanded="false" aria-controls="ocfilter-hidden-values"><?php echo $text_show_all; ?> <i class="fa fa-angle-down"></i></a></p>
							<?php } ?>
						<?php } ?>
