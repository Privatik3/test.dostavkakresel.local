<div id="callback">
	...
	<div>
	<h4 class="heading"><span><?php echo $reason ? $reason : $lang['text_callback']; ?></span></h4>
	<form class="callback">
		<input type="hidden" name="type" value="<?php echo $reason ? $reason : $lang['text_callback']; ?>" />
		<div>
			<input type="text" name="customer_name" value="" placeholder="<?php echo $name_text; ?>" class="form-control"/>
		</div>
		<?php if($show_phone) { ?>
			<div><input type="tel" name="customer_phone" value="" placeholder="<?php echo $phone_text; ?>" class="customer_phone form-control" /></div><?php } ?>
		<?php if($show_email) { ?>
		...
		<?php } ?>
		<?php if($show_comment) { ?>
			<div class="comment" <?php if ($reason) { echo 'style="display:block"'; } ?>>
				<textarea name="customer_comment" placeholder="<?php echo $comment_text; ?>" class="form-control" <?php if (!$reason) { echo 'disabled="disabled"'; } ?> /></textarea>
			</div>
		<?php } ?>
		...
		<div><button type="button" class="callback_button btn btn-primary" data-loading-text="<?php echo $lang['text_loading']; ?>"><?php echo $lang['text_send']; ?></button></div>
	</form>
	</div>
